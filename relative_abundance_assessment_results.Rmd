...
## TODO 
## - define error rate as (exp - obs)/exp - not absolute value, better fit and allows for positive and negative errors to cancel eachother out. 
library(nlme)
library(ape)
library(multcomp)
library(tidyverse)
...
When accounting for biological replicate effect DADA2 had a lower feature-level error rate compared to Mothur and QIIME, and method was lower than QIIME (p-value <1e-09).  
...
The feature-level RCOV was lower for Mothur compared to DADA2 and QIIME though not significantly (Fig. \@ref(fig:relAbuError)C). 
...
## Features with relative abundance estimates and expected values less than 1e-7, these are features that we would not expect to consistently observe in a PCR replicate for the given sequencing depth 
...
error_post_hoc <- glht(error_fit, linfct=mcp(pipe="Tukey")) 

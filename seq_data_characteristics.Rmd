...
qual_fig <- qa_cycle_q_df %>% 
      ...
      geom_smooth(aes(weight = count, group = Read, color = biosample_id)) +
      theme_bw() +
...
feat_fig <- obs_feat_df %>% ggplot() + 
      geom_boxplot(aes(x = pipe, y = obs_feat)) + theme_bw() + 
      labs(x = "Pipeline", y = "# of Features")


ggarrange(lib_size_fig, feat_fig, qual_fig, ncol = 1, nrow = 3,labels = "AUTO")
```

...
To validate the volumetric mixing of the two-sample titration ERCC plasmids were spiked into the unmixed pre- and post-exposure samples. 
The plasmids were quantified using qPCR. 
...
Slope estimates less than 1 were attributed to the assay standard curve amplification factors being less than 2. 
...
The change in pre-exposure sample proportion between titrations 5, 10, and 15 ($0.03$ - $3.05e^-5$) is to small for qPCR to accuratey detect changes in ERCC spike-in concentration. 
For the ERCCs spiked into the pre-treatment samples the $R^2$ values were low, less than 0.6 with slope estimates between -1.5 and -2.1 (Table \@ref(tab:erccTable)) when a regression line is fit to the Ct values and log2 Pre-treatment sample proportion \@ref(fig:erccPlot), with a -1 expected slope. 
These results indicate that the unmixed pre- and post-exposure samples were volumetrically mixed according to the mixture design.
...
ercc_tbl %>% arrange(treatment) %>% 
      rename(Individual = biosample_id, 
             Treatment = treatment, 
             `Std. R2` = std_R2) %>% 
    knitr::kable(caption = "ERCC Spike-in qPCR summary statistics. $R^2$, Efficiency (E), and amplification factor (AF) for standard curves. $R^2$ and slope for titration qPCR results for the titration series.", booktabs = TRUE)
```

```{r erccPlot, fig.cap = "qPCR ERCC spike-in results for ERCC spiked into unmixed (A) Post-exposure samples and (B) Pre-exposure samples. X-axis is on a log2 scale with expected slope of 1 and -1 for Post-exposure and Pre-exposure spike-ins respectively.", fig.height = 6, echo=FALSE, message=FALSE}

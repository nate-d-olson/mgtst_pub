...
Volumetric mixing of the two-sample titration was validated using qPCR to quantify ERCC plasmids spiked into the PRE and POST samples.
...
For the PRE ERCCs, a regression line was fit to the $log_2$ PRE sample proportion for titrations 1-4 and the unmixed PRE sample.
The change in PRE sample proportion between titrations 5, 10, and 15 ($0.97$ - $0.99997$) is too small for qPCR to detect changes in ERCC spike-in concentration with an expected Ct difference of `r round(log2((1-2^-8)/(1-2^-5)),2)` between the titrations 5 and 15.
A regression line was fit to the Ct values and $log_2$ PRE sample proportion.
The expected slope of the regression line is -1, as the spike-in concentration should increase linearly with the proportion of PRE sample and both Ct and PRE sample proportions are on $log_2$ scales. ERCCs spiked into the PRE samples the $R^2$ values were low, less than 0.6, with slope estimates between -1.5 and -2.1 (Table \@ref(tab:erccTable)).
Deviation from the expected slope for the PRE ERCC qPCR results is attributed to the small change in spike-in concentration between samples preventing accurate quantification of changes in spike-in concentration between titrations.
When considering the quantitative limitations of the qPCR assay these results confirm that the unmixed PRE and POST samples were volumetrically mixed according to the design.

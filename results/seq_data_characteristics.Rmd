...
The resulting count tables generated using the four bioinformatic pipelines were characterized for number of features, sparsity, and filter rate (Table \@ref(tab:pipeQA), Fig. \@ref(fig:qaPlots)C).
The pipelines evaluated employ different approaches for handling low quality reads resulting in the large variability in drop-out rate, the fraction of raw sequences not included in the count table (Table \@ref(tab:pipeQA)).
QIIME pipeline has the highest drop-out rate and the highest number of features per sample.

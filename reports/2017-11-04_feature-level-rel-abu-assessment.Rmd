...
```{r}
## Need to figure out why NA
unmix_counts %>% filter(!is.na(error_cat), 
                        median_rel_abu != 0, 
                        !is.na(var_mean)) %>% 
      ggplot() + 
...
```{r}
unmix_counts %>% 
      filter(!is.na(error_cat), !is.na(var_mean)) %>% 
      ggplot() + 
      geom_boxplot(aes(x = error_cat, y = var_mean, fill = error_cat), shape = 21) + 
      theme_bw() + scale_y_log10()
```


```{r}
unmix_counts %>% filter(!is.na(rcov_cat), !is.na(var_mean), rcov_error != 0) %>% 
      ggplot() + 
...
unmix_counts %>% filter(!is.na(rcov_cat), !is.na(var_mean), rcov_error != 0) %>% 
      ggplot() + 
      geom_boxplot(aes(x = rcov_cat, y = var_mean, fill = rcov_cat), shape = 21) + 
      theme_bw() + scale_y_log10()
```
...
```{r}
ec11 %>% filter(logCPM > 7, T1 %in% 0:5, T2 %in% 0:5)
```
...
ec11 %>% filter(logCPM > 7, T1 %in% 0:5, T2 %in% 0:5) %>% 
      ggplot() + 
      geom_point(aes(x = logCPM, y = logFC), 
                 color = "grey60", alpha = 0.25) + 
      # geom_hline(aes(yintercept = -2)) +
      geom_hline(aes(yintercept = 0), linetype = 2) +
      geom_hline(aes(yintercept = as.numeric(T2) - as.numeric(T1))) +
      facet_grid(T1~T2) + 
      theme_bw()
```
...
ec11 %>% filter(logCPM > 7, T1 %in% 0:5, T2 %in% 0:5) %>% 
      ...
      # geom_hline(aes(yintercept = -2)) +
      geom_vline(aes(xintercept = as.numeric(T1) - as.numeric(T2)),
                 linetype = 2) +
      facet_grid(T2~T1, scale = "free_y") + 
      theme_bw()
```

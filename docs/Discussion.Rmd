...
Outlier features, those with extreme quantitative analysis bias and variance metrics, were observed for all pipelines in both relative and differential abundance assessments.
Outlier features could not be attributed to bioinformatic pipelines and are likely due to biases in the molecular biology part of the measurement process not captured in this mixture dataset.
...
Features with outlying metric values were not limited to any specific taxonomic group or phylogenetic clade.
...
Additional research is needed to establish if outlier features can be attributed to mismatches in the primer binding regions.
...
Features present only in the titrations or unmixed samples not due to random sampling are bioinformatic pipeline artifacts as they should be precluded by our experimental design.
...
The DADA2 feature inference algorithm may be aggressively grouping lower abundance true, non-erroneous, sequences with higher abundance sequences.
As a result, the low abundance sequences are not present across samples leading to increased sparsity and higher abundance unmixed- and titration-specific features.
Adjusting the DADA2 parameters, specifically the `OMEGA_A` parameter in `setDadaOpt` may address this observed effect as the DADA2 documentation states that the default setting for `OMEGA_A` is conservative to prevent false positives at the cost of increasing false negatives [@callahan2016dada2].
...
The 16S rRNA region sequenced in the study is larger than the region for which the _de-novo_, and open clustering pipelines were initially developed, potentially explaining the higher than expected sparsity [@kozich2013development].
@kozich2013development were reduced the sequence error rate from 0.29\% to 0.06\% when using paired-end reads that completely overlap.
...
There were also limitations using our mixture dataset, including:
lack of agreement between the estimated proportion of unmixed samples in titrations and the mixture design and the fact that the number of features used in the different analysis varies.
...
Using more diverse endpoint samples to generate mixtures would address this issue.
...
In general, based on our evaluation results we recommend using for DADA2 for feature-level abundance analysis, e.g. differential abundance testing.
While DADA2 performed poorly in our qualitative assessment, the pipeline had performed better in the quantitative assessment compared to the other pipelines.
...
Pipeline parameter optimization could address DADA2 the observed false-negative issue.

...
Targeted sequencing of the 16S rRNA gene, 16S rRNA marker-gene-surveys, is a commonly used method for characterizing microbial communities, microbiomes.
...
The former are used to selectively target and sequence the 16S rRNA gene from prokaryotic organisms within a sample while the latter convert the raw sequence data into a matrix with feature (e.g., operational taxonomic units) relative abundance values for each sample [@Goodrich2014].
...
Appropriate datasets and methods are needed to evaluate the 16S rRNA marker-gene-survey measurement process accuracy, variability and robustness. 
A sample or dataset with "ground truth" is needed to characterize measurement process accuracy, while environmental samples over complex microbial communities, may be used to assess variability and robustness.
...
As a first application of this assessment framework, the mixture datasets were processed using three bioinformatic pipelines and newly developed metrics for qualitative and quantitative assessment of the bioinformatic pipeline results were applied.

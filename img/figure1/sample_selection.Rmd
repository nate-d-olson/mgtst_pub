...
## Sample Selection
```{r}
load("~/Projects/mgtst_pub/data/raw/etecExp.rda")
## needed for mypar
...
pre_post <- bind_rows(qpcr_df, ngs_df) %>% 
    ...
    mutate(time = as.numeric(time),
           pre_post = ifelse(time == -1,"Pre Exposure",""),
           pre_post = ifelse(time == post_sample[ID],
                             "Post Exposure", pre_post))
...
ggplot(ts_df, aes(x = time, y = count)) + geom_point(color = "grey60") +
    ...
    labs(x = "Day Relative to Exposure", y = "Count", 
         color = "Time Point Used")
```

```{r}
ggsave("sample_selection.pdf", width = 7.25, height = 3)
```

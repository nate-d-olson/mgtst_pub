...
We assessed the quantitative and qualitative characteristics of count tables generated using different bioinformatic pipelines using a 16S rRNA marker-gene survey mixture dataset.
The mixture dataset followed a two-sample titration mixture design, where DNA collected before and after exposure to pathogenic _Escherichia coli_ from five vaccine trial participants (subjects) were mixed following a $log_2$ dilution series.
...
We compared feature relative abundance and differential abundance estimates to expected values to quantitatively assess count tables.
Relative abundance expected values were estimated using information from the unmixed samples and mixture design.
The differential abundance expected values were derived using only mixture design information.

### Count Table Assessment Demonstration
We demonstrate our novel assessment approach using count tables generated using four different bioinformatic pipelines.
The pipelines used represent the two most commonly used, Mothur and QIIME, as well as the sequence inference pipeline DADA2.
The Mothur pipeline uses _de novo_ clustering for feature inference [__MOTHUR__].
...
The QIIME pipeline uses open-reference clustering where merged paired-end reads are first assigned to reference cluster centers [__QIIME__].
...
The DADA2 pipeline uses a probability model and maximization expectation algorithm for feature inference [__DADA2__].
...
Mismatches in the primer binding regions impact PCR efficiency and are a potential cause for poor feature-specific performance __REF__.
...
A high rate of false negative features is a potential explanation for the  DADA2 count table's poor performance in the qualitative assessment and comparable sparsity to the other pipelines despite having significantly fewer features [__FIG&TABLE__].
...
Along these lines, the DADA2 documentation states that the default setting for `OMEGA_A` is conservative to prevent false positives at the cost of increasing false negatives[__DADA__].

False positive features provide an explanation for Mothur and QIIME pipelines having lower proportion of unmixed- and titration-specific features not explained by sampling but high sparsity [_FIG&Table__].
...
Mothur and QIIME count tables have ten times and three times more features compared to DADA2, respectively [__TABLE__].
...
Similarly, significantly more features than expected are commonly observed for mock community benchmarking studies evaluating the QIIME and Mothur pipelines [__REF__].
...
The larger region has a smaller overlap between the forward and reverse reads. As a result merging of the forward and reverse reads did not allow for the sequence error correction that occurs when a smaller amplicon is used (__REF-Schloss__). <!-- But it's illumina, I'm surprised if this is a true issue.  What level of sequencing correction is presumed/found typically? -->
However, even when targeting smaller regions of the 16S rRNA gene both the _de-novo_ (Mothur) and open-reference clustering (QIIME) pipelines produced count tables with significantly more features than expected in evaluation studies using mock communities __REF__.
Prevalence filtering is used to exclude low abundance features, likely predominantly measurement artifacts [__Callahan__].
...
Digital PCR provides a more precise alternative to qPCR and is, therefore, a more appropriate method [__REF__].
...
However, the high level of similarity between the unmixed samples resulted in a limited number of features that could be used in the quantitative assessment results [__Supplemental Tables__].

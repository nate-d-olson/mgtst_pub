...
#### Read Counts   
Two barcoded experimental sample has less than 50,000 reads \@ref(fig:readCount). The rest of the samples with less than 50,000 reads are negative PCR controls (NTC). Sample E01JH0016 titration 5 position F9 of plate 1 initial 16S PCR failed. 
...
```{r count_tbl}
meta_df  %>% filter(biosample_id != "NTC",reads < 50000) %>% 
    select(biosample_id, titration, pos, plate, Read, reads) %>% 
    spread(Read, reads) %>% 
...
__NOTE__ Not sure whether or not to include in publication as supplemental material.

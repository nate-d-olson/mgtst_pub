...
agg_curve_df <- agg_curve_df %>% 
      mutate(Treatment = if_else(dilution == 0, "Pre", "Post"))

curve_df %>% mutate(Treatment = if_else(dilution == 0, "Pre", "Post")) %>% 
      ggplot() + 
      ...
      facet_grid(sampleID~pipe) + #, scale = "free") + 
...
Overall samples group together more by treatment status than biological sample. 
...
subset_ps <- function(ps_obj){
      ...
      ps_subset <- subset_samples(ps_obj, dilution %in% c(-1,0)) 
...
pl_list <- map2(subset_obj, subset_ord, plot_ordination, 
                type = "sample",color = "sampleID", shape = "dilution")
```

```{r}
pl_list$dada2 + ggtitle("DADA2 Ordination") + theme_bw()
```

```{r}
pl_list$mothur + ggtitle("Mothur Ordination") + theme_bw()
``` 

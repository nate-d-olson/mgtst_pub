...
```{r}
# pre_post_count <- count_df %>% filter(dilution %in% c(-1,0)) %>% 
#       mutate(unmixed = if_else(dilution == 0, "post", "pre")) %>% 
#       select(-dilution, -samID) %>% 
#       spread(unmixed, count)
...
count_exp_obs %>% filter(norm_method %in% c("csslog","tsslog")) %>% 
      ggplot() + geom_point(aes(x = count, y = exp_count, 
                                color = factor(dilution)), alpha = 0.5) + 
      geom_abline(aes(intercept = 0, slope = 1)) + 
      facet_wrap(pipe~norm_method, scales = "free") + 

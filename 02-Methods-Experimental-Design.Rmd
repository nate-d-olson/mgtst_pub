...
Five trial participants were selected based as those with no _Eshechichia coli_ detected in stool samples before exposure to Enterotoxigenic _Escherichia coli_ (ETEC)) and timepoints with the highest concentration of _E. coli_ after exposure [@pop2016individual](__TODO__ Figure 1).
...
A total of 192 PCRs were run including four PCR replicates per sample and 12 no template controls \@ref(fig:experimental_design).
...
The normalized indexed samples were pooled and sequenced on a single Illumina MiSeq (Illumina Inc., San Diego, CA) run.  
